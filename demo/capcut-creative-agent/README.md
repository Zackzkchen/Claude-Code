...
1. 点右上角 **一键演示** —— Agent 把「脚本 → 人物图/场景图/商品图 → 2 个视频镜头 → 成片」整条链路画在画布上，任务卡里的产出节点可点击定位。
2. **把鼠标移到任意卡片上**：卡片上方浮出一条图标凸巴，下方浮出「标题 ← 来源」。静止时卡片上只有内容本身，功能都藏在这里。鼠标停在图标上 0.2 秒会显示它是什么。
3. **右键**任意卡片，拿到与该类型匹配的功能（多选右键则是「合成为一条成片」，空白右键是新建节点 / 整理布局）。
4. 选中脚本卡片 → 凸巴 **✦** → **文生图**：文本本身就是提示词，所以点了**立刻开始生成**，出现占位卡 + 进度。
5. 选中生成好的图 → **图生视频**：这次会**先落一个空视频节点**，把刚才那张图作为**参考图**挂进它的提示词框，光标已经就位；补一句「缓慢推近，人物转身」再点生成。空节点和虚线连线在点击瞬间就出现，血缘先立住。
6. 双击图片 / 视频卡片（或凸巴上的 **⤢**）打开**局部放大预览**：就是把这块画布放大，右侧详情栏放提示词、素材尺寸、血缘路径。双击文本卡片则是改字。
